...
void Asteroids::update(const Ship &ship, float deltaTime) {
  for (auto &asteroid : m_asteroids) {
    ...
    //if (asteroid.m_translation.y > +1.0f) asteroid.m_translation.y -= 2.0f;
    if (asteroid.m_translation.x < -0.999f){
      asteroid.m_translation.x += 0.1f
      asteroid.m_velocity.x = -asteroid.m_velocity.x * 0.99f;
    } 
    if (asteroid.m_translation.x > +0.999f) {
      asteroid.m_translation.x -= 0.1f
      asteroid.m_velocity.x = -asteroid.m_velocity.x * 0.99f;
    ...
    if (asteroid.m_translation.y < -0.999f) {
      asteroid.m_translation.y += 0.1f
      asteroid.m_velocity.y = -asteroid.m_velocity.y * 0.99f;
    }
    if (asteroid.m_translation.y > +0.999f) {
      asteroid.m_translation.y -= 0.1f
      asteroid.m_velocity.y = -asteroid.m_velocity.y * 0.99f;
...
Asteroids::Asteroid Asteroids::createAsteroid(glm::vec2 translation,
  ...
  asteroid.m_velocity = glm::normalize(direction) / 1.0f;
