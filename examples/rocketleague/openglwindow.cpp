...
void OpenGLWindow::initializeGL() {
  ...
  car.loadModelFromFile(getAssetsPath() + "car/car.obj");
  car.initializeGL(m_program);

  field.loadModelFromFile(getAssetsPath() + "field/field.obj");
...
void OpenGLWindow::update() {
  ...
  if (lockCamera) {
    m_camera.lookAtCar(
      glm::vec3(car.x(), car.y(), car.z()),
      glm::vec3(ball.x(), ball.y(), ball.z())
    );

    car.move(m_dollySpeed, m_panSpeed, deltaTime);
    return;
  }
